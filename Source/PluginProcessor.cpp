    ...
    for (int i = 0; i < mySynth.getNumVoices(); i++) {
        ...
        if ((myVoice = dynamic_cast<SynthVoice*>(mySynth.getVoice(i)))) {
            ...
            myVoice->getOscParams(
                ...
                apvts.getRawParameterValue("detune1"),
                apvts.getRawParameterValue("detune2"));
...
LEMSynthAudioProcessor::createParameters() {
    ...
    params.push_back(std::make_unique<juce::AudioParameterInt>("detune2", "Detune Shift 2", -100, 100, 0));
    // Envelope's parameters
    params.push_back(std::make_unique<juce::AudioParameterFloat>("attack", "Attack", juce::NormalisableRange<float>(5.f, 5000.f, 1.f, 0.25f), 1.f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>("decay", "Decay", juce::NormalisableRange<float>(5.f, 5000.f, 1.f, 0.25f), 5.f));
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>("release", "Release", juce::NormalisableRange<float>(5.f, 5000.f, 1.f, 0.25f), 5.f));
    // Filter's parameters
    ...
    // Noise's parameters
    params.push_back(std::make_unique<juce::AudioParameterChoice>("noiseType", "Noise Type", juce::StringArray("LPF2ord", "BPF2ord", "HPF2ord", "LPF1ord", "HPF1ord", "LPShelf", "HPShelf", "Peak"), 0));
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>("noiseFreq", "Noise Freq", juce::NormalisableRange<float>(30.f, 20000.f, 1.f, 0.4f), 80.f));
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>("noiseGain", "Noise Gain", juce::NormalisableRange<float>(-60.f, 18.f, 0.1f), -3.f));
